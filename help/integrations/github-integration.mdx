...
<Tip>
  Use fine-grained tokens where possible for better security. Fine-grained
  tokens allow more granular permission control.
</Tip>
...
<Info>
  ...
  GitHub accounts. User-level integrations are also available in **Settings** →
...
<Check>
  The GitHub username must be valid and the account must be accessible with the
...
<Tip>
  ...
  guidance about GitHub integration and account linking.
</Tip>
