...
<Info>
  The onboarding system helps new team members become effective quickly through
  structured, trackable onboarding journeys. Create reusable templates, assign
  them to people, and track progress as they complete each step.
</Info>
...
<Card title='Instances' icon='clipboard-check'>
  Live onboarding assigned to specific people. Tracks their progress through the
  template.
</Card>

<Card title='Phases' icon='layers'>
  Organized groupings of items (Day 1, Week 1, etc.) that structure the
  onboarding journey.
</Card>
...
<Tip>
  Create templates that match your actual onboarding process. Start with your
  most common onboarding path, then create variations for different roles or
  teams.
</Tip>
...
<Info>
  Phases are organizational tools. They don't enforce timing—you can complete
  items from multiple phases if needed. But they help structure the journey.
</Info>
...
<Tip>
  Start with your most common onboarding path. You can always create variations
  later for different roles or teams.
</Tip>
...
<Info>
  The system will suggest templates based on the person's team and job role. If
  no match is found, it will suggest the organization default template.
</Info>
...
<Check>
  Most items can be completed by the onboardee. CHECKPOINT items require manager
  or mentor confirmation to ensure quality.
</Check>
...
<Warning>
  Only managers and admins can mark onboarding as complete. This ensures quality
  and prevents premature completion.
</Warning>
...
<Tip>
  **Start simple, iterate** Create a basic template first, then refine it based
  on feedback. You can always add more phases and items later.
</Tip>
