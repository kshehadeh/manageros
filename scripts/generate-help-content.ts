...
function parseFrontMatter(content: string): {
  ...
  if (!frontMatter.id || !frontMatter.title || !frontMatter.category) {
    console.warn('⚠️  Missing required front matter fields:', frontMatter)
    return { frontMatter: null, markdown: content }
  }
...
function generateHelpContent() {
  console.log('🔄 Generating help content from hierarchical markdown files...')
  ...
  try {
    ...
    for (const { filePath, relativePath } of markdownFiles) {
      try {
        ...
        if (!frontMatter) {
          console.warn(`⚠️  No valid front matter found in: ${relativePath}`)
          continue
        }
...
`
        ...
        console.error(`❌ Error reading help file ${relativePath}:`, error)
      }
    }
