...
export async function POST(req: Request) {
  ...
  // Verify webhook signature - this can throw if signature is invalid or headers are missing
  try {
    evt = await verifyWebhook(req)
  } catch (error) {
    console.error('Webhook verification failed:', error)
    return new Response('Invalid webhook signature or missing headers', {
      status: 400,
    })
  }
