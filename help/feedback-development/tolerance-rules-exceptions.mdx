...
Tolerance rules monitor important organizational practices like one-on-one frequency, initiative check-ins, feedback cycles, and management span. When a threshold is exceeded, the system creates an **exception** that you can view and manage on the Exceptions page.
...
<Card title='Actionable Alerts' icon='bell'>
  When thresholds are exceeded, exceptions are created with clear information
  about what needs attention. View them on the Exceptions page.
</Card>
...
1. **Review the details** - Open the Exceptions page and click through to see what needs attention
...
When an exception is created, you can view it on the **Exceptions** page:

- **Severity**: Warning or urgent (based on the rule threshold)
- **Message**: Clear description of what needs attention
- **Entity link**: Navigate to the related person, initiative, or feedback campaign to take action
...
### Not Seeing Exceptions

- Verify you have access to view exceptions (organization member)
- Check the Exceptions page and filter by status (e.g., active)
- Ensure the tolerance rule is enabled and has run (daily or run manually)
