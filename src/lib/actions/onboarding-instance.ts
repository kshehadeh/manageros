...
/**
 * Get a specific onboarding instance by ID
 * Access control: onboardee, manager, mentor, or admin
 */
export async function getOnboardingInstanceById(id: string) {
  const user = await getCurrentUser()
  ...
  // Check access: must be onboardee, manager, mentor, or admin
  const isOnboardee = instance.personId === user.managerOSPersonId
  const isManager = instance.managerId === user.managerOSPersonId
  const isMentor = instance.mentorId === user.managerOSPersonId
  const isAdmin = isAdminOrOwner(user)

  if (!isOnboardee && !isManager && !isMentor && !isAdmin) {
    throw new Error('You do not have permission to view this onboarding')
  }
...
/**
 * Update an onboarding instance (status, manager, mentor)
 */
export async function updateOnboardingInstance(
  id: string,
  data: OnboardingInstanceUpdateData
) {
  const user = await getCurrentUser()
  ...
  // Validate data
  const validatedData = onboardingInstanceUpdateSchema.parse(data)
  ...
  if (!isManager && !isAdmin) {
    throw new Error(
      'Only the onboarding manager or an admin can update this onboarding'
    )
  }

  // Verify new manager if specified
  if (validatedData.managerId) {
    const manager = await prisma.person.findFirst({
      where: {
        id: validatedData.managerId,
  ...
  // Verify new mentor if specified
  if (validatedData.mentorId) {
    const mentor = await prisma.person.findFirst({
      where: {
        id: validatedData.mentorId,
  ...
  if (validatedData.status !== undefined) {
    ...
    if (validatedData.status === 'COMPLETED') {
      ...
      // Clear completedAt for non-completed statuses
  ...
  if (validatedData.mentorId !== undefined) {
    updateData.mentorId = validatedData.mentorId
  }
