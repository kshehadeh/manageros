...
/**
 * Get initiative summary data for modal display
 * Includes: title, summary, status, rag, priority, size, dates, team, owners, task/objective counts, most recent check-in
 */
export async function getInitiativeSummaryForModal(initiativeId: string) {
  try {
    const user = await getCurrentUser()
    ...
    // Check permission to view this initiative
    const hasPermission = await getActionPermission(
      user,
      'initiative.view',
      initiativeId
    ...
    // Get initiative with related data
    const initiative = await prisma.initiative.findFirst({
      where: {
        id: initiativeId,
        organizationId: user.managerOSOrganizationId,
    ...
    // Get task counts and calculate progress
    const allTasks = await prisma.task.findMany({
      where: {
        OR: [
          { initiativeId: initiativeId },
          { objective: { initiativeId: initiativeId } },
        ],
      },
      select: { status: true },
    })
    ...
    const mostRecentCheckIn = await prisma.checkIn.findFirst({
      where: {
        initiativeId: initiativeId,
      },
      select: {
    ...
    return {
      ...
      objectiveCount: initiative._count.objectives,
      ...
      mostRecentCheckIn: mostRecentCheckIn
        ? {
            ...
            weekOf: mostRecentCheckIn.weekOf,
            rag: mostRecentCheckIn.rag,
            confidence: mostRecentCheckIn.confidence,
            summary: mostRecentCheckIn.summary,
            createdAt: mostRecentCheckIn.createdAt,
            createdBy: mostRecentCheckIn.createdBy,
          }
        : null,
    }
  } catch (error) {
